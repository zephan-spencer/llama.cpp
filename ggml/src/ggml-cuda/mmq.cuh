...
#pragma unroll
        ...
        if constexpr (use_cache_source) {
            ...
            x_channel = source_id >= 0 ? source_id : -source_id - 1;
            if (source_id < 0) {
                x_base = source_host;
                x_stride = source_host_stride;
            }
        }
        const int offset_x = fastdiv(wt, sample_ratio)*stride_sample_x + x_channel*x_stride + it*I*stride_row_x;
