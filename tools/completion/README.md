...
| `--moe-cache-experts N` | keep routed expert weights in host memory and cache N logical experts per MoE layer; N must be positive and smaller than the model's expert count; disabled by default<br/>(env: LLAMA_ARG_MOE_CACHE_EXPERTS) |
| `--moe-cache-policy {lru}` | expert cache residency policy (default: lru)<br/>(env: LLAMA_ARG_MOE_CACHE_POLICY) |
