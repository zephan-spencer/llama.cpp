...
enum llama_load_mode llama_load_mode_from_str(const char * str) {
    ...
    if (std::strcmp(str, "mmap") == 0) {
        return LLAMA_LOAD_MODE_MMAP;
    }
    if (std::strcmp(str, "mlock") == 0) {
        return LLAMA_LOAD_MODE_MLOCK;
    }
    if (std::strcmp(str, "mmap+mlock") == 0) {
        return LLAMA_LOAD_MODE_MMAP_MLOCK;
    }
    if (std::strcmp(str, "dio") == 0) {
        return LLAMA_LOAD_MODE_DIRECT_IO;
    }
    ...
    if (params.devices) {
        if (params.split_mode == LLAMA_SPLIT_MODE_TENSOR) {
            ...
            model->devices.push_back(
                { true, ggml_backend_meta_device(params.devices, n_devs, llama_meta_device_get_split_state,
        ...
        if (params.split_mode == LLAMA_SPLIT_MODE_TENSOR) {
            ...
            model->get_split_state_ud.model     = model;
            gpus.push_back({ true, ggml_backend_meta_device(devs.data(), devs.size(), llama_meta_device_get_split_state,
                                                            &model->get_split_state_ud) });
        } else {
            for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
                    ...
                    case GGML_BACKEND_DEVICE_TYPE_GPU:
                        {
                            ...
                            if (ggml_backend_reg_name(reg) == std::string("RPC")) {
                                ...
                                if (it != gpus.end()) {
                                    LLAMA_LOG_INFO(
                                        "%s: skipping device %s (%s) with id %s - already using device %s (%s) with "
                                        "the same id\n",
...
static std::pair<int, llama_model *> llama_model_load(struct gguf_context *         metadata,
    ...
    try {
        llama_model_loader ml(metadata, set_tensor_data, set_tensor_data_ud, fname, splits, file, params.load_mode,
                              params.check_tensors, params.no_alloc, params.load_mtp, params.kv_overrides,
                              params.tensor_buft_overrides, params.n_moe_cache_experts);
...
static struct llama_model * llama_model_load_from_file_impl(struct gguf_context *         metadata,
    ...
    const auto [status, model] =
        llama_model_load(metadata, set_tensor_data, set_tensor_data_ud, path_model, splits, file, params);
...
struct llama_model * llama_model_init_from_user(struct gguf_context *         metadata,
    ...
    return llama_model_load_from_file_impl(metadata, set_tensor_data, set_tensor_data_ud, path_model, splits,
                                           /*file*/ nullptr, params);
}
