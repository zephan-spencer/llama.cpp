...
static void test(void) {
    ...
    argv = {"binary_name", "-m", "model.gguf", "--tensor-read-lazy", "on"};
    ...
    assert(common_model_params_to_llama(lazy_params).lazy_mode == LLAMA_LAZY_MODE_ON);

    common_params cpu_ffn_params;
    argv = {"binary_name", "-m", "model.gguf", "--n-cpu-ffn", "2"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), cpu_ffn_params, LLAMA_EXAMPLE_COMMON));
    assert(cpu_ffn_params.tensor_buft_overrides.size() >= 3);
    assert(std::string(cpu_ffn_params.tensor_buft_overrides[0].pattern) == "blk\\.0\\.ffn_(up|down|gate)\\.");
    assert(std::string(cpu_ffn_params.tensor_buft_overrides[1].pattern) == "blk\\.1\\.ffn_(up|down|gate)\\.");
    assert(cpu_ffn_params.tensor_buft_overrides[2].pattern == nullptr);
