...
static plan_result make_reference(const test_case & test) {
    ...
    result.protected_epoch = test.protected_epoch.empty() ?
        std::vector<uint64_t>(test.n_cache, 0) : test.protected_epoch;
...
static void run_case(const test_case & test, hipStream_t stream) {
    ...
    protected_epoch.set(test.protected_epoch.empty() ?
        std::vector<uint64_t>(test.n_cache, 0) : test.protected_epoch);
    ...
    ggml_cuda_launch_expert_route_tiles(
        saved_route_bounds.ptr, saved_route_tile_bounds.ptr,
        test.n_expert, route_tile_width, stream);
    ...
    for (int32_t expert = 0; expert < test.n_expert; ++expert) {
        ...
        expected_route_tile_bounds[expert + 1] = expected_route_tile_bounds[expert] +
            (routes + route_tile_width - 1)/route_tile_width;
    }
    expect_equal(test.name, "saved_route_tile_bounds", saved_route_tile_bounds.get(), expected_route_tile_bounds, test.n_expert + 1);
    const int32_t route_tile_upper_bound = n_ids == 0 ? 0 :
        (n_ids + route_tile_width - 1)/route_tile_width + std::min(test.n_expert, n_ids) - 1;
...
static void run_deferred_priority_case(hipStream_t stream) {
    ...
    auto invoke = [&](const std::vector<int32_t> & route_ids, const std::vector<int32_t> & route_priority, int32_t route_epoch) {
...
static void run_backend_cached_source_case(ggml_backend_t backend, ggml_type type) {
    ...
    try {
        ...
        ggml_tensor * cache_plan = ggml_custom_4d(
            ...
            cache_args, 5, nullptr, 1, &cache_desc);
...
static void run_backend_interface_lifecycle_case(ggml_backend_t backend) {
    ...
    if (plan.selectors == nullptr || plan.execution == nullptr ||
            ...
            ggml_nelements(plan.execution) != 2*ggml_nelements(ids) + 18) {
        api->destroy(cache);
