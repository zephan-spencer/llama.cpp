...
common_models_handler common_models_handler_init(const common_params & params, llama_example curr_ex) {
    ...
    const bool spec_type_draft_dflash =
    ...
    const bool spec_type_draft_eagle3 =
        std::find(params.speculative.types.begin(), params.speculative.types.end(),
                  COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3) != params.speculative.types.end();

    const bool spec_type_draft_dspark =
        std::find(params.speculative.types.begin(), params.speculative.types.end(),
...
void common_models_handler_apply(common_models_handler &    handler,
                                 common_params &            params,
    ...
    auto add_tasks = [&opts, &tasks](const hf_cache::hf_files & model_files, const hf_cache::hf_file & primary,
    ...
    const bool spec_sidecar_found = !plan_spec.mtp.local_path.empty() || !plan_spec.dflash.local_path.empty() ||
                                    !plan_spec.eagle3.local_path.empty() || !plan_spec.dspark.local_path.empty();
    ...
    if (!plan.mmproj.local_path.empty()) {
        tasks.emplace_back(plan.mmproj, opts, [&]() { params.mmproj.path = hf_cache::finalize_file(plan.mmproj); });
    }
...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    auto parse_cli_args = [&]() {
        ...
        auto has_arg = [&](std::initializer_list<const char *> names) {
            return std::any_of(names.begin(), names.end(), [&](const char * name) { return seen_args.count(name); });
        };
        if (has_arg({ "-lm", "--load-mode" }) &&
            has_arg({ "--mlock", "--mmap", "--no-mmap", "-dio", "--direct-io", "-ndio", "--no-direct-io" })) {
            LOG_WRN(
                "DEPRECATED: `--load-mode` and `--mlock`/`--mmap`/`--direct-io` should not be combined; only the last "
                "flag on the command line will take effect\n");
        }
    };
    ...
    if ((!params.server_tools.empty() || mcp_enabled) && !params.cors_origins_explicit) {
        LOG_WRN(
            "server tools or MCP servers are enabled, using localhost as default CORS origin (change via "
            "--cors-origins)\n");
...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(common_arg({ "-h", "--help", "--usage" }, "print usage and exit", [](common_params & params) {
                params.usage = true;
    ...
    add_opt(common_arg({ "--server-base" }, "URL",
                       string_format("connect to this server instead of starting a new one, example: "
                                     "'http://localhost:8080' (default: none)"),
                       [](common_params & params, const std::string & value) { params.server_base = value; })
    ...
    add_opt(common_arg({ "--repeat-last-n" }, "N",
                       string_format("last n tokens to consider for penalize (default: %d, 0 = disabled)",
    ...
    add_opt(common_arg({ "--repeat-penalty" }, "N",
                       ...
                       [](common_params & params, const std::string & value) {
                           ...
                           if (!std::isfinite(penalty_repeat) || penalty_repeat <= 0.0f ||
    ...
    add_opt(common_arg(
                ...
                .set_sampling());
    add_opt(common_arg({ "--dry-penalty-last-n" }, "N",
                       string_format("set DRY penalty for the last n tokens (default: %d, 0 = disable)",
    ...
    add_opt(common_arg({ "--mlock" },
                       "DEPRECATED in favor of `--load-mode`: force system to keep model in RAM rather than swapping "
                       "or compressing",
                       [](common_params & params) {
    ...
    add_opt(common_arg({ "--mmap" }, { "--no-mmap" },
                       "DEPRECATED in favor of `--load-mode`: whether to memory-map model. (if mmap disabled, slower "
    ...
    add_opt(common_arg({ "-cmoe", "--cpu-moe" }, "keep all Mixture of Experts (MoE) weights in the CPU",
                       [](common_params & params) {
                           params.cpu_moe = true;
    ...
    add_opt(
        common_arg({ "-m", "--model" }, "FNAME",
            ...
            .set_examples(
                { LLAMA_EXAMPLE_COMMON, LLAMA_EXAMPLE_EXPORT_LORA, LLAMA_EXAMPLE_DOWNLOAD, LLAMA_EXAMPLE_TOKENIZE })
    ...
    add_opt(common_arg({ "-hft", "--hf-token" }, "TOKEN",
                ...
                .set_env("HF_TOKEN"));
    add_opt(common_arg(
                { "--mtp" }, "also download the multi-token prediction (MTP) head, if available (default: unused)",
                [](common_params & params) { params.speculative.types.push_back(COMMON_SPECULATIVE_TYPE_DRAFT_MTP); })
                .set_examples({ LLAMA_EXAMPLE_DOWNLOAD }));
    add_opt(common_arg({ "--dflash" }, "also download the DFlash sidecar, if available (default: unused)",
                       [](common_params & params) {
                           params.speculative.types.push_back(COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH);
                       })
                .set_examples({ LLAMA_EXAMPLE_DOWNLOAD }));
    add_opt(common_arg({ "--eagle3" }, "also download the Eagle3 sidecar, if available (default: unused)",
                       [](common_params & params) {
    ...
    add_opt(common_arg({ "-o", "--output", "--output-file" }, "FNAME",
                ...
                .set_examples({ LLAMA_EXAMPLE_IMATRIX, LLAMA_EXAMPLE_CVECTOR_GENERATOR, LLAMA_EXAMPLE_EXPORT_LORA,
                                ...
                                LLAMA_EXAMPLE_EXPORT_GRAPH_OPS, LLAMA_EXAMPLE_CLI }));
    ...
    add_opt(common_arg(
                { "--ids" },
                string_format("only print the token IDs, in a Python-parseable list form like [1, 2, 3] (default: %s)",
                              params.tokenize_ids ? "true" : "false"),
                [](common_params & params) { params.tokenize_ids = true; })
                .set_examples({ LLAMA_EXAMPLE_TOKENIZE }));
    add_opt(common_arg({ "--stdin" },
                       string_format(
                           "read the prompt from stdin (takes precedence over -f/--file and -p/--prompt) (default: %s)",
                           params.tokenize_stdin ? "true" : "false"),
                       [](common_params & params) { params.tokenize_stdin = true; })
                .set_examples({ LLAMA_EXAMPLE_TOKENIZE }));
    add_opt(common_arg(
                { "--no-bos" },
                string_format("do not add a BOS token to the prompt, even if the model normally uses one (default: %s)",
                              params.tokenize_no_bos ? "true" : "false"),
                [](common_params & params) { params.tokenize_no_bos = true; })
                .set_examples({ LLAMA_EXAMPLE_TOKENIZE }));
    add_opt(common_arg({ "--no-parse-special" },
                       string_format("do not parse special tokens (chat, tool, etc) (default: %s)",
                                     !params.parse_special ? "true" : "false"),
                       [](common_params & params) { params.parse_special = false; })
                .set_examples({ LLAMA_EXAMPLE_TOKENIZE }));
    add_opt(common_arg({ "--show-count" },
                       string_format("print the total number of tokens (default: %s)",
                                     params.tokenize_show_count ? "true" : "false"),
                       [](common_params & params) { params.tokenize_show_count = true; })
                .set_examples({ LLAMA_EXAMPLE_TOKENIZE }));
    ...
    add_opt(common_arg({ "--cors-methods" }, "METHODS",
                       string_format("comma-separated list of allowed methods for CORS (default: %s)",
                                     params.cors_methods.c_str()),
                       [](common_params & params, const std::string & value) { params.cors_methods = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_CORS_METHODS"));
    add_opt(common_arg({ "--cors-headers" }, "HEADERS",
                       string_format("comma-separated list of allowed headers for CORS (default: %s)",
                                     params.cors_headers.c_str()),
                       [](common_params & params, const std::string & value) { params.cors_headers = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_CORS_HEADERS"));
    add_opt(common_arg({ "--cors-credentials" }, { "--no-cors-credentials" },
                       string_format("whether to allow credentials for CORS (default: %s)\n"
                                     "note: if this is enabled and --cors-origins is set to * (default), the Origin "
                                     "header will be echoed back, and credentials will always be allowed",
                                     params.cors_credentials ? "enabled" : "disabled"),
                       [](common_params & params, bool value) { params.cors_credentials = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(
        common_arg({ "--tools-runtime" }, "OPTION",
                   "experimental: run tools in a separate runtime environment (default: none, use host environment)\n"
                   "available options:\n"
                   "  'docker:<image>': spin up a new Docker container and reuse it for all invocations, clean up on "
                   "server exit\n"
                   "  'docker-container:<id>': use an existing Docker container by ID, won't stop on server exit\n",
                   [](common_params & params, const std::string & value) { params.server_tools_runtime = value; })
            .set_examples({ LLAMA_EXAMPLE_SERVER })
            .set_env("LLAMA_ARG_TOOLS_RUNTIME"));
    add_opt(common_arg({ "--mcp-servers-config" }, "PATH",
                       "experimental: path to JSON file with MCP server definitions (Cursor-compatible format) - do "
                       "not enable in untrusted environments (default: none)\n"
                       "note: for security reasons, this will limit --cors-origins to localhost by default",
                       [](common_params & params, const std::string & value) { params.mcp_servers_config = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_MCP_SERVERS_CONFIG"));
    add_opt(common_arg({ "--mcp-servers-json" }, "JSON",
                       "experimental: inline JSON with MCP server definitions (Cursor-compatible format) - do not "
                       "enable in untrusted environments (default: none)\n"
                       "note: for security reasons, this will limit --cors-origins to localhost by default",
                       [](common_params & params, const std::string & value) { params.mcp_servers_json = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "--reasoning-preserve" }, { "--no-reasoning-preserve" },
                       "preserve reasoning trace in the full history, not just the last assistant message (default: "
                       "template default)\n"
    ...
    add_opt(
        common_arg({ "--log-prompts-dir" }, "PATH",
    ...
    add_opt(common_arg({ "--tts-lang" }, "FNAME",
                       "language (ISO 639-1) for audio generation\n"
                       "see tts/README.md for per-model usage notes",
                       [](common_params & params, const std::string & value) { params.tts_lang = value; })
                .set_examples({ LLAMA_EXAMPLE_TTS }));
    add_opt(common_arg({ "--tts-speaker-file" }, "FNAME", "speaker file path for audio generation",
                       [](common_params & params, const std::string & value) { params.tts_speaker_file = value; })
                .set_examples({ LLAMA_EXAMPLE_TTS }));
